...
{
    //远程主机ip
    private static String host = "ftp-1.fuiou.com";

    //远程主机ip
    private static Integer port = 9021;

    //远程主机用户名
    private static String account = "FTP999999";

    //远程主机密码
    private static String password = "CttPIyBE6fW6LrgR";

    //重试次数
    private static String reTryTimes = "3";
    ...
    public static boolean connectToFtp()
    {
        for(int i = 0;i< Integer.valueOf(reTryTimes);i++)
        {
            ...
            log.error("method:FtpUtil.connectToFtp():failure times :" + (i+1) +",host is " + host);
        }
    ...
    {
        {
            ...
            try
            {
                if(!client.isConnected())
                {
                    ...
                    {
                        log.debug("method:FtpUtil.connectToFtpProcess():login " +(flag.toString().equals("true") ? "seccuss!" : "fail!"));
                    }
                }
                ...
                if(!flag)
                {
                    ...
                    log.error("method:FtpUtil.connectToFtpProcess():login failed:userName:" +account);
            ...
            {
                log.error("method:FtpUtil.connectToFtpProcess(): connectToFtp, failed,userName:" + account +",\nexception:"+e );
                return false;
            }
            return true;
        }
    ...
    public static void closeResources(InputStream is, FileOutputStream os,FTPClient client)
    {
        try
        {
            ...
            if(client.isConnected())
            {
                ...
                if(log.isDebugEnabled())
                {
                    log.debug("method:FtpUtil.closeResources(): user was logout");
                }
                ...
                if(log.isDebugEnabled())
                {
                    log.debug("method:FtpUtil.closeResources(): ftp was disconnected");
                }
            }
        }
        catch (IOException e)
        {
            log.error("method: Ftputil.closeResources:closeResources:fail,\n the status of ftpClient："+(String.valueOf(client.isConnected()).equals(Boolean.TRUE)?"connect":"disconnect"+ ",\nexception:"+e));
        }
    }
